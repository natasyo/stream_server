...
export class ProfileService {
    ...
    public async changeAvatar(user: User, file: FileUpload) {
        ...
        const processBuffer = await sharp(buffer, {animated: isGif}).resize(512, 512).webp().toBuffer()
        await this.storageService.uploadFile(fileName, processBuffer, 'webp/image')
    ...
    public async createSocialLink(user:User, input:SocialLinkInput){
        ...
        await this.prismaService.socialLink.create({
            data:{
                title, position:newPosition, url,user:{
                    connect:{
                        id:user.id
                    }
                }
